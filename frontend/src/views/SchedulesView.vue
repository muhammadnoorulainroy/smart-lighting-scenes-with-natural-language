<template>
  <div class="schedules-page container mx-auto px-4 py-8">
    ...
    <div class="card p-6 mb-8">
      ...
      <div class="flex items-center gap-4">
        <div class="flex-1 relative">
          <input v-model="nlpCommand" type="text" class="input w-full pr-12" placeholder="Try: 'Turn off all lights at 11pm every day'" @keyup.enter="processNlpCommand" />
          <button class="absolute right-2 top-1/2 -translate-y-1/2 p-2 text-neutral-500 hover:text-primary-500" :class="{ 'text-red-500 animate-pulse': isListening }" @click="toggleVoiceInput">
      ...
      <div v-if="nlpResult" class="mt-4 p-4 rounded-lg" :class="nlpResult.valid ? 'bg-green-50 dark:bg-green-900/20' : 'bg-red-50 dark:bg-red-900/20'">
        <div v-if="nlpResult.valid" class="space-y-3">
          <p class="font-medium">{{ nlpResult.preview }}</p>

          <!-- Conflict Warning -->
          <div v-if="nlpResult.conflictAnalysis?.hasConflicts" class="mt-4 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-300 dark:border-amber-700 rounded-lg">
            <div class="flex items-center gap-2 mb-3">
              <svg class="w-5 h-5 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
              <span class="font-semibold text-amber-700 dark:text-amber-300">Schedule Conflict Detected</span>
            </div>

            <p class="text-sm text-amber-700 dark:text-amber-300 mb-4">{{ nlpResult.conflictAnalysis.summary }}</p>

            <div v-for="(conflict, idx) in nlpResult.conflictAnalysis.conflicts" :key="idx" class="mb-4 p-3 bg-white dark:bg-neutral-800 rounded border border-amber-200 dark:border-amber-800">
              ...
              <p class="text-sm text-neutral-600 dark:text-neutral-400 mb-3">{{ conflict.description }}</p>

              <div class="space-y-2">
                <p class="text-xs font-medium text-neutral-500 uppercase">Choose a Resolution:</p>
                <div v-for="resolution in conflict.resolutions" :key="resolution.id" class="flex items-center gap-2">
                  <input
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div v-for="schedule in schedules" :key="schedule.id" class="card p-5" :class="{ 'opacity-50': !schedule.enabled }">
        <div class="flex items-start justify-between">
          ...
          <div class="flex flex-col gap-2 items-end">
            ...
            <div class="flex gap-1">
              <button class="p-2 hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded" @click="editSchedule(schedule)">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                </svg>
              </button>
              <button class="p-2 hover:bg-red-100 dark:hover:bg-red-900/30 rounded text-red-500" @click="deleteSchedule(schedule)">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const toggleSchedule = async schedule => {
  try { await schedulesApi.toggle(schedule.id); schedule.enabled = !schedule.enabled }
...
const processNlpCommand = async () => {
  ...
  try {
    ...
    if (nlpResult.value.valid && !nlpResult.value.isScheduled) {
      nlpResult.value = { valid: false, error: 'This looks like an immediate command. Add a time like "at 7pm".' }
    }
  } catch (err) {
...
const toggleVoiceInput = () => {
  ...
  recognition.onresult = e => { nlpCommand.value = e.results[0][0].transcript; isListening.value = false; processNlpCommand() }
...
const formatTrigger = schedule => {
  ...
  if (schedule.triggerType === 'time') {
    ...
    if (!days || days.length === 7) {return `Every day at ${time}`}
    if (days.length === 5 && !days.includes('sat') && !days.includes('sun')) {return `Weekdays at ${time}`}
