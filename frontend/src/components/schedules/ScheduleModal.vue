        ...
        <form class="space-y-4" @submit.prevent="handleSubmit">
          ...
          <div v-if="formData.triggerType === 'sun'" class="space-y-3">
            ...
            <div>
              ...
              <input
                v-model.number="formData.offsetMinutes"
                type="number"
                class="input w-full"
                placeholder="e.g., -30 for 30 min before"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-2">
              <button
                ...
                :key="day.value"
                type="button"
                class="px-3 py-1 rounded-full text-sm border-2 transition-colors"
                :class="
          ...
          <div v-if="formData.actionType === 'light.brightness'">
            <label class="block text-sm font-medium mb-1"
              >Brightness: {{ formData.brightness }}%</label
            >
            <input
...
const toggleDay = day => {
  ...
  if (idx >= 0) {
    formData.value.selectedDays.splice(idx, 1)
  } else {
...
const handleSubmit = () => {
  const triggerConfig =
    formData.value.triggerType === 'time'
  ...
  if (formData.value.actionType === 'light.brightness') {
    action.params = { brightness: formData.value.brightness }
  }
  if (formData.value.actionType === 'scene.apply') {
    action = { type: 'scene', scene_id: formData.value.sceneId }
  }
...
onMounted(async () => {
  try {
    availableScenes.value = await scenesApi.getAll()
  } catch (err) {
