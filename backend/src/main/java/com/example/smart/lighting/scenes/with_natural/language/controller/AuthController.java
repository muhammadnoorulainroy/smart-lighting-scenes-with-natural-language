...
public class AuthController {
    ...
    /**
     * Exchange JWT token for session (used for cross-domain OAuth on Render).
     * The token is generated by OAuth2AuthenticationSuccessHandler and passed via URL.
     */
    ...
    public ResponseEntity<?> exchangeToken(
        ...
        try {
            ...
            if (user == null) {
                log.warn("Token exchange failed: user not found for ID {}", userId);
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
